...
//=============================================================================
// $File: //depot/3darg/Tools/NormalMapper/NormalMapper.cpp $ $Revision: 1.1 $ $Author: hax22 $
//=============================================================================
...
//we are sending not triagle pixels, but triangle bounding box pixels
//so this value should be 10x large then desired number of computed texels
#define MIN_PIXELS_TO_SEND (512*10)
...
static const char mutexName[] = "AtiNormalMapperMutex";
...
static DWORD gTrisDone = 0;
static int gLowNumTris = 1;
static int gl = 0;
static DWORD g_lastProcessTriIndex = 0;


static IGenericStream* globalDataStream;
...
//  method to construct the 3Ч3 matrix that rotates a unit vector f into
//  another unit vector t, rotating about the axis fЧt. We give experimental
...
static DWORD* triIdImg;
static DWORD* triIdImg2;


//=================================================================
//=================================================================
void __cdecl ProcessReturnedData(IGenericStream* resStream)
{
 //todo: running in background thread
 //need critical section, since using incremeting and comparing
 ...
 resStream->Read(&img,sizeof(float*));
 resStream->Read(&img2,sizeof(float*));
 resStream->Read(&l,sizeof(l));

 BYTE ostag;
 DWORD osindex;
 float osval;

 resStream->Read(&ostag,sizeof(ostag));
  ...
  switch (ostag)
   {
    ...
    case 1:
    {
     resStream->Read(&osindex,sizeof(osindex));
     resStream->Read(&osval,sizeof(osval));

     if (l>=triIdImg[osindex])
      {
       img[osindex]=osval;
       triIdImg[osindex]=l;
     }
 ...
 DWORD tricount;
 resStream->Read(&tricount,sizeof(tricount));

 gTrisDone+=tricount;

 {
 char prog[256];
 float progress1 = ((float)(gTrisDone)/(float)(gLowNumTris))*100.0f;
 sprintf (prog, "%6.2f%%", progress1);
 printf ("\r%s", prog);
 }
...
//=======================================================================================================
//=======================================================================================================
void Process(IGridUser* gridUser, IGenericStream* dataStream, DWORD& pixelsCounter, bool& bFirstPass, DWORD curTriIndex)
{
 pixelsCounter=0;
 bFirstPass=true;
 ...
 {
  DWORD tag = 0;
  dataStream->Write(&tag, sizeof(DWORD));
 }
 
 {
  //write number of triangles, processed by this task
  DWORD d = curTriIndex - g_lastProcessTriIndex;
  g_lastProcessTriIndex = curTriIndex; 
  
  dataStream->Write(&d, sizeof(DWORD));
 }

 dataStream->Seek(0);

#ifdef LOCALRUN                     
 IGenericStream* resStream = new TGenericStream(50000);
 
 RunTask(dataStream, resStream);
 dataStream->Release();

 ProcessReturnedData(resStream); 
 resStream->Release();
#else
 DWORD dummyTaskId;
 gridUser->RunTask("NormalMapperTask.dll","RunTask", dataStream, &ProcessReturnedData, &dummyTaskId); 
#endif               
// gridUser->WaitForCompletion();
}
...
{
 assert(strcmp(dataDesc,"NormalMapperData")==0);
...
main (int argc, char **argv)
{
 EnableMemoryLeaksReport(MEMORYLEAKS_BASIC);
  ...
  NmPrint ("Distributed computing modification v1.0beta by Roman Lut [hax@deep-shadows.com]\n");
  NmPrint ("-------------------------------------------------------------------------------\n");
 ...
 for (int i=0; i<argc; i++)
  ...
  NmPrint( "\nWaiting while other instances of NormalMapper finish to work...\n" );  

  bool sincSucc = appSync.Do( mutexName, SyncWaitCallback );
  if ( !sincSucc )
  {
   NmPrint( "\n Multi-application mutex sync failed" );  
   _getch();
  }

  NmPrint ("Connecting to grid...\n");

 IGridUser* gridUser = NULL;
#ifndef LOCALRUN                     
 gridUser = CreateGridUserObject(IGridUser::VERSION);
 
 gridUser->BindGetDataCallback(GetDataCallback);
 
 {
  DWORD refCount = gridUser->AddRef();
  refCount = gridUser->Release();
 }
#endif 

   //очередность выполенния задач не гарантирована. 
   //Поскольку мы хотим получить в точности те же результаты, что и при 
   //singlethreaded, заведем  triangeIdImg, куда будем писать id треугольника, 
   //который поставил пиксел.
   //Ставить разрешается только если id>=already_set_id

   // loop over mip levels
   int mipCount = 0;
   int lastWidth = gWidth;
   int lastHeight = gHeight;
   DWORD pixelsCounter = 0;
   IGenericStream* dataStream;
   bool bFirstPass = true;

   NmPrint ("Creating global data...\n");
   
   //---------------------------------------------------------------------
   //prepare global data
   
   //corretly approximate stream size
   //we do not want stream to grow and reallocate - can reach limit of virtual memory very fast
   
   DWORD approxSize = octree->ApproximateSize();
   
   approxSize += highNumTris*sizeof(NmRawTriangle);
   approxSize += gNumSamples*sizeof(NmSample);

   if (hTangentSpace!=NULL)
    {
     approxSize += highNumTris*sizeof(NmTangentMatrix);
    }
   if (bumpMap!=NULL)
    {
     approxSize += bumpWidth*bumpHeight*3*sizeof(float);
    }

   if (numRays!=0)
    {
     approxSize += numRays*sizeof(NmRawPointD);
     approxSize += numRays*sizeof(double);
    }
    
   approxSize += 1024*100; 
   
   globalDataStream = new TGenericStream(approxSize,100000000);
   
   octree->Serialize(globalDataStream);

   globalDataStream->Write(&highNumTris,sizeof(highNumTris));
   globalDataStream->Write(highTris,highNumTris*sizeof(NmRawTriangle));

   globalDataStream->Write(&gNumSamples,sizeof(gNumSamples));
   globalDataStream->Write(gSamples,gNumSamples*sizeof(NmSample));

   globalDataStream->Write(&hTangentSpace,sizeof(hTangentSpace));
   if (hTangentSpace!=NULL)
    {
     globalDataStream->Write(hTangentSpace,highNumTris*sizeof(NmTangentMatrix));
    }

   globalDataStream->Write(&bumpWidth,sizeof(bumpWidth));
   globalDataStream->Write(&bumpHeight,sizeof(bumpHeight));
   globalDataStream->Write(&bumpMap,sizeof(bumpMap));
   
   if (bumpMap!=NULL)
    {
     globalDataStream->Write(bumpMap,bumpWidth*bumpHeight*3*sizeof(float));
    }

   globalDataStream->Write(&numRays,sizeof(numRays));
   if (numRays!=0)
    {
     globalDataStream->Write(rays,numRays*sizeof(NmRawPointD));
     globalDataStream->Write(rayWeights,numRays*sizeof(double));
    }
    
   delete octree;
   delete[] gSamples;
   delete[] highTris;
   delete[] hTangentSpace;
   delete[] bumpMap;
   delete[] rays;
   delete[] rayWeights;
   ...
   HeapCompact((PVOID)hCrtHeap, HEAP_NO_SERIALIZE);
   gridUser->CompressStream(globalDataStream);
   ...
   while ((gWidth > 0) && (gHeight > 0))
   {
      ...
      if ((gComputeMipLevels == MIP_RECOMPUTE) || (mipCount == 0))
      {
         ...
         triIdImg = new DWORD[gWidth*gHeight*numComponents+numComponents];
         triIdImg2 = new DWORD[gWidth*gHeight*numComponents+numComponents];

         memset(triIdImg, 0, (gWidth*gHeight*numComponents+numComponents)*sizeof(DWORD));
         memset(triIdImg2, 0, (gWidth*gHeight*numComponents+numComponents)*sizeof(DWORD));
         ...
         for (int l = 0; l < lowNumTris; l++)
         {
            ...
            if (gQuiet == NM_VERBOSE)
            {
               float progress1 = ((float)(gTrisDone)/(float)(lowNumTris))*100.0f;
               float progress2 = ((float)(l+1)/(float)(lowNumTris))*100.0f;
               sprintf (prog, "%6.2f%%  (sent: %6.2f%%)", progress1, progress2);
               printf ("\r%s", prog);
            }
            ...
            for (int y = firstY; y <= lastY; y++)
            {
               ...
               if (bFirstPass==true)
               {
                dataStream = new TGenericStream(20000000);
               }
               ...
               dataStream->Write(&img, sizeof(float*));
               dataStream->Write(&img2, sizeof(float*));
               
               dataStream->Write(&minX, sizeof(minX));
               dataStream->Write(&pad, sizeof(pad));
               dataStream->Write(&maxX, sizeof(maxX));
               dataStream->Write(&gWidth, sizeof(gWidth));
               dataStream->Write(&y, sizeof(y));
               
               dataStream->Write(&x1, sizeof(x1));
               dataStream->Write(&x2, sizeof(x2));
               dataStream->Write(&x3, sizeof(x3));

               dataStream->Write(&y1, sizeof(y1));
               dataStream->Write(&y2, sizeof(y2));
               dataStream->Write(&y3, sizeof(y3));
               
               dataStream->Write(&b0, sizeof(b0));

               dataStream->Write(&gInTangentSpace, sizeof(gInTangentSpace));
               
               dataStream->Write(&tangentSpace[l],sizeof(NmTangentMatrix));

               dataStream->Write(lTri,sizeof(NmRawTriangle));

/*               
               if (bFirstPass==true)
               {
                 octree->Serialize(dataStream);

                 dataStream->Write(&highNumTris,sizeof(highNumTris));
                 dataStream->Write(highTris,highNumTris*sizeof(NmRawTriangle));

                 dataStream->Write(&gNumSamples,sizeof(gNumSamples));
                 dataStream->Write(gSamples,gNumSamples*sizeof(NmSample));

                 dataStream->Write(&hTangentSpace,sizeof(hTangentSpace));
                 if (hTangentSpace!=NULL)
                  {
                   dataStream->Write(hTangentSpace,highNumTris*sizeof(NmTangentMatrix));
                  }

                 dataStream->Write(&bumpWidth,sizeof(bumpWidth));
                 dataStream->Write(&bumpHeight,sizeof(bumpHeight));
                 dataStream->Write(&bumpMap,sizeof(bumpMap));
                 
                 if (bumpMap!=NULL)
                  {
                   dataStream->Write(bumpMap,bumpWidth*bumpHeight*3*sizeof(float));
                  }

                 dataStream->Write(&numRays,sizeof(numRays));
                 if (numRays!=0)
                  {
                   dataStream->Write(rays,numRays*sizeof(NmRawPointD));
                   dataStream->Write(rayWeights,numRays*sizeof(double));
                  }
               }
*/


               dataStream->Write(&gOcclusion,sizeof(gOcclusion));
               dataStream->Write(&gBentNormal,sizeof(gBentNormal));
 

               dataStream->Write(&numComponents,sizeof(numComponents));

               dataStream->Write(&gOcclIdx,sizeof(gOcclIdx));

               dataStream->Write(&gDisplacements ,sizeof(gDisplacements));

               dataStream->Write(&gDispIdx ,sizeof(gDispIdx));

               dataStream->Write(&gEdgeCopy, sizeof(gEdgeCopy));

               dataStream->Write(&gNormalRules, sizeof(gNormalRules));

               dataStream->Write(&gMaxAngle, sizeof(gMaxAngle));

               dataStream->Write(&gDistance, sizeof(gDistance));

               dataStream->Write(&gEpsilon, sizeof(gEpsilon));

               dataStream->Write(&l, sizeof(l));
...
//-------------------------
               ...
               pixelsCounter += (maxX + pad) - (minX - pad) + 1; 
               
               DWORD mulpix = (gOcclusion || gBentNormal)?1:10;

               if (pixelsCounter>=MIN_PIXELS_TO_SEND*mulpix)
                {
                 Process(gridUser, dataStream, pixelsCounter, bFirstPass, l);
                }
...
*/
        if (pixelsCounter>0)
        {
         Process(gridUser, dataStream, pixelsCounter, bFirstPass, lowNumTris-1);
        }
...
*/
   ...
   NmPrint ("Done in %d seconds.\n",(GetTickCount()-d)/1000);

#ifndef LOCALRUN
  DWORD refCount = gridUser->Release();
#endif

   NmPrint ("GridUser released.\n");
